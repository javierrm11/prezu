...
export default async function PresupuestosPage() {
  ...
  const presupuestos: PresupuestoFila[] = filas.map((fila) => ({
    ...
    cliente: fila.clientes?.nombre ?? fila.cliente_nombre ?? "Sin cliente",
    concepto: fila.presupuesto_lineas[0]?.concepto ?? "",
  }));

  return <ListaPresupuestos presupuestos={presupuestos} seriePresupuesto={seriePresupuesto} />;
}
